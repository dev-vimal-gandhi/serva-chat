  ...
  private fun updateStatus(status: AutomaticVerificationStatus) {
    when (status) {
      ...
      AutomaticVerificationStatus.UNAVAILABLE_PERMANENT -> {
        ...
        ImageViewCompat.setImageTintList(binding.autoVerifyIcon, ColorStateList.valueOf(ThemeUtil.getThemedColor(requireContext(), MaterialR.attr.colorOnSurfaceVariant)))
