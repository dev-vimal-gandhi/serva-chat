...
import com.servalabs.chat.database.model.addLink
  ...
  private fun updateReleaseNotes(announcements: List<ReleaseNote>) {
    ...
    val resolvedNotes: List<FullReleaseNote?> = announcements
      ...
      .filter {
        ...
        if (minVersion != null) {
          minVersion > values.highestVersionNoteReceived && minVersion <= BuildConfig.SIGNAL_CANONICAL_VERSION_CODE
        } else {
          false
        }
      }
