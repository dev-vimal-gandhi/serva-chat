...
class SignalLogDetector : Detector(), Detector.UastScanner {
  ...
  override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
    ...
    if (evaluator.isMemberInClass(method, "com.servalabs.chat.glide.Log")) {
      context.report(
        issue = LOG_NOT_SIGNAL,
        scope = node,
        location = context.getLocation(node),
        message = "Using 'com.servalabs.chat.glide.Log' instead of a Signal Logger",
        quickfixData = quickFixIssueLog(node)
      )
    }
    ...
    if (evaluator.isMemberInClass(method, "com.servalabs.chat.core.util.logging.Log")) {
  ...
  private fun quickFixIssueLog(logCall: UCallExpression): LintFix {
    ...
    var fixSource = "com.servalabs.chat.core.util.logging.Log."
